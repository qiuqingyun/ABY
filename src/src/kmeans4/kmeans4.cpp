...
//对每个簇，找出其质心
void findCenters(std::vector<std::vector<uint64_t>> list, std::vector<uint64_t *> &centers, std::vector<uint32_t> classInfo,
                 std::vector<uint64_t *> &oldCenters, uint32_t cluster,
...
void eDistance(std::vector<std::vector<uint64_t>> list, std::vector<uint64_t *> centers, std::vector<uint32_t> &classInfo,
               e_role role, uint32_t dimension, const std::string &address, uint16_t port, seclvl seclvl,
               uint32_t bitlen, uint32_t nthreads, e_mt_gen_alg mt_alg)
{
    ABYParty *party = new ABYParty(role, address, port, seclvl, bitlen, nthreads, mt_alg, 4000000);
            ...
            uint32_t bitlen, uint32_t nthreads, e_mt_gen_alg mt_alg)
{
    ...
    std::vector<uint64_t *> centers;    //质心向量
    std::vector<uint64_t *> oldCenters; //上一轮的质心向量
    for (int i = 0; i < cluster; i++)
    { //初始化填充
        uint64_t *temp1 = new uint64_t[dimension];
        uint64_t *temp2 = new uint64_t[dimension];
